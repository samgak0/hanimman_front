...
const host = `${jwtAxios.defaults.baseURL}/api/v1/share-review`;

export const createReview = async (shareReviewDTO) => {
  try {
    const response = await jwtAxios.post(`${host}/create`, shareReviewDTO);
...
export const updateReview = async (id, shareReviewDTO) => {
  try {
    const response = await jwtAxios.put(`${host}/${id}`, shareReviewDTO);
    return response.data;
  } catch (error) {
